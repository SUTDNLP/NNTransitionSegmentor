/*
 * LSTMNABeamSearcher.h
 *
 *  Created on: Jan 25, 2016
 *      Author: mszhang
 */

#ifndef SRC_LSTMNABeamSearcher_H_
#define SRC_LSTMNABeamSearcher_H_
...
public:
		...
		while (true) {
			...
			for (pGenerator = lattice_index[index - 1]; pGenerator != lattice_index[index]; ++pGenerator) {
				...
				for (tmp_j = 0; tmp_j < actions.size(); ++tmp_j) {
					...
					scored_action.nnfeat._actionRNNHidden = 0.0;
	...
	void backPropagationStates(const CStateItem<xpu> *pPredState, const CStateItem<xpu> *pGoldState, dtype predLoss, dtype goldLoss,
      ...
      /*
			...
			for(int idx = 0; idx < predDenseFeature._actionHiddenLoss.size(); idx++){
				predDenseFeature._actionHiddenLoss[idx] = 0.0;
			}			
     ...
     /*
			...
			for(int idx = 0; idx < goldDenseFeature._actionHiddenLoss.size(); idx++){
				goldDenseFeature._actionHiddenLoss[idx] = 0.0;
			}					
...
#endif /* SRC_LSTMNABeamSearcher_H_ */
