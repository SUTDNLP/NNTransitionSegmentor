...
#ifndef SRC_TNNBeamSearcher_H_
#define SRC_TNNBeamSearcher_H_
...
public:
	...
	UniLayer<xpu> _char_tnn;
	UniLayer<xpu> _word_tnn;
	UniLayer<xpu> _action_tnn;
...
public:
	...
	inline void init(const NRMat<dtype>& wordEmb, const NRMat<dtype>& allwordEmb, const NRMat<dtype>& lengthEmb, int wordNgram, int wordHiddenSize, int wordRNNHiddenSize,
		...
		_charTNNHiddenSize = charRNNHiddenSize;
		...
		_sep_hiddenInSize = _wordRNNHiddenSize + _actionRNNHiddenSize + _charTNNHiddenSize;
		_app_hiddenInSize = _actionRNNHiddenSize +  _charTNNHiddenSize;
		...
		_char_tnn.initial(_charTNNHiddenSize, _charHiddenSize, true, 30);
		_word_tnn.initial(_wordRNNHiddenSize, _wordHiddenSize, true, 50);
		_action_tnn.initial(_actionRNNHiddenSize, _actionHiddenSize, true, 60);
	...
	dtype trainOneExample(const std::vector<std::string>& sentence, const vector<CAction>& goldAC, int num) {
		 ...
		 */
		charFeat.init(length, _charDim, _biCharDim, _charcontext, _charHiddenSize, _charTNNHiddenSize, 0, true);
		...
		_char_tnn.ComputeForwardScore(charFeat._charHidden, charFeat._charLeftRNNHidden);
		...
		while (true) {
			...
			for (pGenerator = lattice_index[index - 1]; pGenerator != lattice_index[index]; ++pGenerator) {
				...
				for (tmp_j = 0; tmp_j < actions.size(); ++tmp_j) {
					...
					scored_action.nnfeat.init(_wordDim, _allwordDim, _charDim, _lengthDim, _wordNgram, _wordHiddenSize, _wordRNNHiddenSize, _actionDim, _actionNgram, _actionHiddenSize,
							_actionRNNHiddenSize, _sep_hiddenInSize, _app_hiddenInSize, _sep_hiddenOutSize, _app_hiddenOutSize, 0, true);
					...
					_action_tnn.ComputeForwardScore(scored_action.nnfeat._actionHidden, scored_action.nnfeat._actionRNNHidden);
					...
					if (scored_action.action._code == CAction::SEP || scored_action.action._code == CAction::FIN) {
						...
						_word_tnn.ComputeForwardScore(scored_action.nnfeat._wordHidden, scored_action.nnfeat._wordRNNHidden);
			...
			if (!bCorrect) {
				...
				pGoldFeat.init(correctState->_wordnum, index, _wordDim, _allwordDim, _charDim, _lengthDim, _wordNgram, _wordRepresentDim, _wordHiddenSize, _wordRNNHiddenSize, _actionDim, _actionNgram, _actionHiddenSize,
						_actionRNNHiddenSize);
				backPropagationStates(pBestGen, correctState, 1.0/num, -1.0/num, charFeat._charLeftRNNHidden_Loss, charFeat._charRNNHiddenDummy_Loss, pBestGenFeat,
						pGoldFeat);

				_char_tnn.ComputeBackwardLoss(charFeat._charHidden, charFeat._charLeftRNNHidden, charFeat._charLeftRNNHidden_Loss, charFeat._charHidden_Loss);
		...
		if (pBestGen != correctState) {
			...
			//std::cout << index << " updated" << std::endl;
			//std::cout << "best score: " << pBestGen->_score << " , gold score: " << correctState->_score << std::endl;
			pBestGenFeat.init(pBestGen->_wordnum, index, _wordDim, _allwordDim, _charDim, _lengthDim, _wordNgram, _wordRepresentDim, _wordHiddenSize, _wordRNNHiddenSize, _actionDim, _actionNgram, _actionHiddenSize, _actionRNNHiddenSize);
			pGoldFeat.init(correctState->_wordnum, index, _wordDim, _allwordDim, _charDim, _lengthDim, _wordNgram, _wordRepresentDim, _wordHiddenSize, _wordRNNHiddenSize, _actionDim, _actionNgram, _actionHiddenSize, _actionRNNHiddenSize);
			backPropagationStates(pBestGen, correctState, 1.0/num, -1.0/num, charFeat._charLeftRNNHidden_Loss, charFeat._charRNNHiddenDummy_Loss, pBestGenFeat,
					pGoldFeat);
			pBestGenFeat.clear();
			pGoldFeat.clear();
	...
	void backPropagationStates(const CStateItem<xpu> *pPredState, const CStateItem<xpu> *pGoldState, dtype predLoss, dtype goldLoss,
			Tensor<xpu, 3, dtype> charLeftRNNHidden_Loss, Tensor<xpu, 2, dtype> charRNNHiddenDummy_Loss,
		...
		if (pPredState->_nextPosition == 0) {
			//predState
			_word_tnn.ComputeBackwardLoss(predDenseFeature._wordHidden, predDenseFeature._wordRNNHidden, predDenseFeature._wordRNNHiddenLoss, predDenseFeature._wordHiddenLoss);
			...
			_action_tnn.ComputeBackwardLoss(predDenseFeature._actionHidden, predDenseFeature._actionRNNHidden, predDenseFeature._actionRNNHiddenLoss, predDenseFeature._actionHiddenLoss);
			...
			//goldState
			_word_tnn.ComputeBackwardLoss(goldDenseFeature._wordHidden, goldDenseFeature._wordRNNHidden, goldDenseFeature._wordRNNHiddenLoss, goldDenseFeature._wordHiddenLoss);
			...
			_action_tnn.ComputeBackwardLoss(goldDenseFeature._actionHidden, goldDenseFeature._actionRNNHidden, goldDenseFeature._actionRNNHiddenLoss, goldDenseFeature._actionHiddenLoss);
		...
		if (pPredState != pGoldState) {
			...
			if (pPredState->_lastAction._code == CAction::SEP || pPredState->_lastAction._code == CAction::FIN) {
				...
				unconcat(predDenseFeature._actionRNNHiddenLoss[position], charLeftRNNHidden_Loss[position], pPredState->_nnfeat._appInHiddenLoss);
			}
			...
			if (pGoldState->_lastAction._code == CAction::SEP || pGoldState->_lastAction._code == CAction::FIN) {
				...
				unconcat(goldDenseFeature._actionRNNHiddenLoss[position], charLeftRNNHidden_Loss[position], pGoldState->_nnfeat._appInHiddenLoss);
			}
	...
	bool decode(const std::vector<string>& sentence, std::vector<std::string>& words) {
		...
		/*
		 Add Character bi rnn  here
		 */
		charFeat.init(length, _charDim, _biCharDim, _charcontext, _charHiddenSize, _charTNNHiddenSize);
		int unknownCharID = fe._charAlphabet[fe.unknownkey];
		for (int idx = 0; idx < length; idx++) {
			charFeat._charIds[idx] = fe._charAlphabet[sentence[idx]];
			if (charFeat._charIds[idx] < 0)
		...
		while (true) {
			...
			for (pGenerator = lattice_index[index - 1]; pGenerator != lattice_index[index]; ++pGenerator) {
				...
				for (tmp_j = 0; tmp_j < actions.size(); ++tmp_j) {
					...
					scored_action.nnfeat.init(_wordDim, _allwordDim, _charDim, _lengthDim, _wordNgram, _wordHiddenSize, _wordRNNHiddenSize, _actionDim, _actionNgram, _actionHiddenSize,
							_actionRNNHiddenSize, _sep_hiddenInSize, _app_hiddenInSize, _sep_hiddenOutSize, _app_hiddenOutSize);
	...
	void updateParams(dtype nnRegular, dtype adaAlpha, dtype adaEps) {
		...
		_char_tnn.updateAdaGrad(nnRegular, adaAlpha, adaEps);
		_word_tnn.updateAdaGrad(nnRegular, adaAlpha, adaEps);
		_action_tnn.updateAdaGrad(nnRegular, adaAlpha, adaEps);
